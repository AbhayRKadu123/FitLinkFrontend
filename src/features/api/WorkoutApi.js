...
export const WorkoutApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    GetCustomWorkoutFolders:builder.query({
      query:() =>`/workout/GetCustomWorkoutFolders`
    }),
    // useAddCustomWorkoutAddFolderMutation
    //  AddCustomWorkoutAddFolder:builder.query({
    //   query:() =>`/workout/CustomWorkoutAddFolder`
    // }),
    AddCustomWorkoutAddFolder:builder.mutation({
      query: ({name}) => ({
        url: `/workout/CustomWorkoutAddFolder`,
        method: "POST",
         body: { name }
      }),
...
AddProgressPhoto: builder.mutation({
 ...
 query: ({SessionTitle,Currexercise,Day}) => `/Workout/GetLastSessionHistory?SessionTitle=${SessionTitle}&Currexercise=${Currexercise}&Day=${Day}`
    ...
    GetCurrentexerise:builder.query({
      // Currentexerise
       query: ({ name }) => `/workout/Currentexerise?name=${name}`}),
